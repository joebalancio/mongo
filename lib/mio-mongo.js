...
module.exports = function (settings) {
  return function (Resource) {
    ...
    Resource
      .before('findOne', createEventHandler('findOne'))
      .before('find', createEventHandler('find'))
      .before('count', createEventHandler('count'))
      .before('save:new', createEventHandler('create'))
      .before('save:update', createEventHandler('update'))
      .before('remove', createEventHandler('destroy'))
      .before('updateMany', createEventHandler('updateMany'))
      .before('removeMany', createEventHandler('destroyMany'));
  };
};
...
exports.find = function (query, next) {
  ...
  Resource.mongo.collection.find(mongoQuery, options)
...
function whereQuery (query) {
  if (!query.where) {
    ...
    for (var key in query) {
      if (!key.match(/(where|limit|skip|offset|page|sort)/)) {
...
function filterByID (resource) {
  ...
  filter[attr.alias || primaryKey] = new ObjectID(resource.primary);
