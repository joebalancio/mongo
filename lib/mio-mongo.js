...
/**
 * It is recommended to share the same `settings.client` object between
 ...
 * `Resource.options.mongo.client` and once connected the collection will be
 * available via `Resource.mongo`.
 *
 *
...
module.exports = function createMioMongoPlugin(settings) {
  return function MioMongoPlugin(Resource) {
    if (!settings.client) {
      ...
      if (loaded) {
        console.warn(
          "[warning] resources should share a MongoClient to "
...
exports.destroy = function (query, next) {
  this.mongo.remove(query, function (err, result) {
    next(err);
  });
};
...
function connect (Resource, done) {
  ...
  var mongo = Resource.options.mongo.client;

  if (mongo.connected) {
    if (!Resource.mongo) {
      Resource.mongo = mongo.db.collection(settings.collection);
    }
    done.call(Resource);
    ...
    mongo.connect(settings.url, settings.options, function(err, db) {
      ...
      Resource.mongo = db.collection(settings.collection);
      mongo.db = db;
