...
exports.create = function (body, next, resource) {
  ...
  mongoExec(Resource, 'insert')(
    namesToAliases(scrubBody(body, attributes), attributes),
    { w: 1 },
    function (err, result) {
      if (err) return next(err);
...
exports.update = function (query, changes, next, resource) {
  var Resource = this;
  var attributes = Resource.attributes;
  var doc = namesToAliases(changes, attributes);

  delete doc[attributes[Resource.primaryKey].alias || Resource.primaryKey];

  mongoExec(Resource, 'findAndModify')(
    namesToAliases(query.where(), attributes),
    [],
    { $set: scrubBody(doc, attributes) },
    { 'new': true },
    function (err, result) {
      if (err) return next(err);
...
exports.updateMany = function (query, changes, next) {
  ...
  mongoExec(this, 'update')(
    namesToAliases(query.where(), attributes),
    {
      $set: scrubBody(namesToAliases(changes, attributes), attributes),
    },
    options,
    function (err, result) {
      next(err);
...
/**
 * Returns body scrubbed of undefined, unserializable, and relation attributes.
 *
 * @param {Object} body
 * @param {external:mio.Resource.attributes} attributes
 * @returns {Object}
 */
function scrubBody(body, attributes) {
  var scrubbed = {};

  for (var key in attributes) {
    var attr = attributes[key];
    var lookup = attr.alias || key;

    if (attr && !attr.relation && attr.serializable !== false) {
      if (body[lookup]) {
        scrubbed[lookup] = body[lookup];
      }
    }
  }
 ...
 * @private
 */
function namesToAliases(query, attributes, parent) {
  ...
  for (var key in query) {
    ...
    if (value && typeof value === 'object' && value.constructor !== ObjectID) {
      namesToAliases(value, attributes, ((value.$in && key) || parent));
    }
  }
