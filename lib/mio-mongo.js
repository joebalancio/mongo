...
exports.findOne = function (query, done) {
  ...
  Object.keys(where).forEach(function (key) {
    ...
    var unpackedWhere = unpack(where);

    if (relation) {
      var nestedAttr = nestedName && relation.target.attributes[nestedName];
      var nestedRelation = nestedAttr && nestedAttr.relation;
      var whereRelation = unpackedWhere[relationName];

      if (nestedRelation) {
        var nestedWhere = unpackedWhere[relationName][nestedName];
...
exports.find = function (query, done) {
  ...
  Object.keys(query.where()).forEach(function (key) {
    var keys = key.split('.');
    ...
    if (relation) {
      var unpackedWhere = unpack(query.where());
